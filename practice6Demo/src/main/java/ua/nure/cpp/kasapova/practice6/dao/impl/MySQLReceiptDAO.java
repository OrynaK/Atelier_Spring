package ua.nure.cpp.kasapova.practice6.dao.impl;

import ua.nure.cpp.kasapova.practice6.dao.DAOConfig;
import ua.nure.cpp.kasapova.practice6.dao.DAOFactory;
...
public class MySQLReceiptDAO implements ReceiptDAO {
    ...
    public List<Receipt> loadAll() throws DBException {
        try (Connection con = DAOConfig.getConnection(true)) {
    ...
    @Override
    public void update(Receipt receipt) throws DBException {
        try (Connection con = DAOConfig.getConnection(true)) {
            try (PreparedStatement st = con.prepareStatement(UPDATE_RECEIPT)) {
                ...
                st.setInt(++k, receipt.getFabricId());
                st.setInt(++k, receipt.getCutterId());
                st.setDate(++k, receipt.getDateAcceptance());
                st.setDate(++k, receipt.getDateFitting());
                st.setDate(++k, receipt.getDueDate());
                st.setBoolean(++k, receipt.getStatus());
                st.setInt(++k, receipt.getId());
                st.executeUpdate();
            }
        } catch (SQLException e) {
            throw new DBException(e);
